...
def compressImage(image):
    ...
    # conversion de l'image (la matrice) en un tableau
    imageCompressed = compress(input.flatten())
    compressedImage.extend(imageCompressed)
...
def main():
    ...
    if arguments.action == 'compress':
        if arguments.mode == 'text' :
            ...
            input = Image.open(ABSOLUTE_PATH+"//"+arguments.input)
